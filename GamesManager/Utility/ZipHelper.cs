...
namespace GamesManager.Utility
{
    ...
    {
        public static string GetUnityVersionShitFuck(string zipfileName)
        {
            ZipFile zf = null;
            ...
            try
            {
                ...
                ZipEntry z_t = zf.GetEntry("assets/bin/Data/mainData");

                if (z_t != null)
                {
                    Stream stream_t = zf.GetInputStream(z_t);
                    StreamReader sr = new StreamReader(stream_t);

                    int tryCount = 0;

                    string t = sr.ReadLine().Trim();
                    while (!t.Contains(".") && (!t.Contains("f") || !t.Contains("p")) && tryCount < 3)
                    {
                        t = sr.ReadLine().Trim();
                        tryCount++;
                    }

                    int index_n = t.IndexOf(".");
                    ...
                    t = t.Substring(index_n - 1, 5).Trim();
